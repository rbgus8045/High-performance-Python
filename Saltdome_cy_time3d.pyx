...
cdef int nt=len(w)
cdef np.float_t dt=0.002
cdef np.float_t h=0.02
cdef int order=8
...
cdef np.ndarray vel=np.fromfile("R_SEG_C3NA_Velocity.km.bin",dtype=np.float32)
...
seismo.tofile("1cy_20m_3dsalt.bin")
